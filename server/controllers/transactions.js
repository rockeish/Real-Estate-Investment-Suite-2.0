...
exports.getTransactionById = async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await db.query('SELECT * FROM financial_transactions WHERE id = $1', [id]);
    if (rows.length === 0) {
...
exports.createTransaction = async (req, res) => {
  ...
  if (!property_id || !transaction_type || !category || !amount || !transaction_date) {
  ...
  try {
    const { rows } = await db.query(
      ...
      [property_id, transaction_type, category, description, amount, transaction_date]
    );
...
exports.updateTransaction = async (req, res) => {
  ...
  if (!property_id || !transaction_type || !category || !amount || !transaction_date) {
    return res.status(400).json({ error: 'Missing required fields for transaction update.' });
  }
  ...
  try {
    const { rows } = await db.query(
      ...
      [property_id, transaction_type, category, description, amount, transaction_date, id]
    );
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Transaction not found' });
    }
...
exports.deleteTransaction = async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await db.query('DELETE FROM financial_transactions WHERE id = $1 RETURNING *', [id]);
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Transaction not found' });
    }
