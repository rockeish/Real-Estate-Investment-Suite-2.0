...
exports.getUnitById = async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await db.query('SELECT * FROM property_units WHERE id = $1', [id]);
    if (rows.length === 0) {
...
exports.createUnit = async (req, res) => {
  ...
  if (!property_id || !unit_label) {
    return res.status(400).json({ error: 'property_id and unit_label are required' });
  }
...
exports.updateUnit = async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      ...
      [property_id, unit_label, bedrooms, bathrooms, square_feet, market_rent, current_rent, occupied, id]
    );
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Unit not found' });
    }
...
exports.deleteUnit = async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await db.query('DELETE FROM property_units WHERE id = $1 RETURNING *', [id]);
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Unit not found' });
    }
