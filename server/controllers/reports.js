...
exports.getSummaryReport = async (req, res) => {
  try {
    ...
    const tenantsQuery = 'SELECT COUNT(*) FROM tenants'
    const leasesQuery = 'SELECT COUNT(*) FROM leases WHERE is_active = TRUE'
    const paymentsQuery =
      'SELECT SUM(amount_paid) FROM rent_payments WHERE paid_date IS NOT NULL'
    ...
    const [
      ...
      db.query(propertiesQuery),
      db.query(tenantsQuery),
      db.query(leasesQuery),
      db.query(paymentsQuery),
    ])

    const summary = {
      total_properties: parseInt(propertiesResult.rows[0].count, 10),
      total_tenants: parseInt(tenantsResult.rows[0].count, 10),
      active_leases: parseInt(leasesResult.rows[0].count, 10),
      total_rent_collected: parseFloat(paymentsResult.rows[0].sum) || 0,
    }
